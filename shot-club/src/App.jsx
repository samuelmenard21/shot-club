...
export default function ClubScreen() {
  ...
  // Load stats, teams, lifetime leaderboard once club resolves
  useEffect(() => {
    if (!club) return
    let cancelled = false
  ...
  // Today leader — refetched on a heartbeat for live-ish feel
  useEffect(() => {
    if (!club) return
    let cancelled = false
    ...
    const fetchTodayLeader = async () => {
      try {
        ...
        // Get all player ids in this club
        ...
        // Pulse the dot when value changes
        setTodayPulse(true)
        setTimeout(() => setTodayPulse(false), 600)
      } catch (e) {
        // Silent — public page, don't break the screen on transient errors
      }
    }
  ...
  return (
    <div className="club fade-in">
      ...
      {/* Today strip — live-ish heartbeat */}
      {todayLeader ? (
      ...
      {/* Empty state — the most important state */}
      {!hasAnyShots && (
        <div className="empty">
      ...
      {hasAnyShots && (
        <section className="rank-section">
          ...
          {top1 && (
            <Link to={`/card/${top1.username || ''}`} className="hero-card">
...
function RankRow({ rank, player }) {
  ...
  return (
    <Link to={`/card/${player.username || ''}`} className="rank-row">
