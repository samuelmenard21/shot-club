...
export default function RankScreen() {
  ...
  // Determine default scope based on what player has
  useEffect(() => {
    if (!player) return
    if (player.team_id) setScope('team')
    else if (player.club_name) setScope('club')
    else setScope('global')
  }, [player?.team_id, player?.club_name])

  // Build the current scope params
  const scopeParams = useMemo(() => {
    if (!player || !scope) return {}
    if (scope === 'team') return { teamId: player.team_id }
    if (scope === 'club') return { clubName: player.club_name }
    return {}
  }, [player, scope])
  ...
  useEffect(() => {
    ...
    fetcher(scopeParams).then((r) => {
      setRows(r)
      setLoading(false)
    })

    if (scope === 'team' && player.team_id) {
      getTeamSize(player.team_id).then(setScopeSize)
    } else if (scope === 'club' && player.club_name) {
      getClubSize(player.club_name).then(setScopeSize)
    } else {
      setScopeSize(0)
    }
  }, [player, scope, period, scopeParams])
  ...
  if (!player || !scope) return null

  const teamLabel = player.team?.name || 'Team'
  const clubLabel = player.club_name || 'Club'

  return (
    <div className="rank-screen fade-in">
      <header className="rank-header">
        <h1 className="rank-title">Rankings</h1>
      </header>

      {/* Scope selector - 3 tabs */}
      <div className="seg-control">
        <button
          className={`seg-btn ${scope === 'team' ? 'seg-btn--active' : ''}`}
        ...
        <button
          className={`seg-btn ${scope === 'club' ? 'seg-btn--active' : ''}`}
          onClick={() => setScope('club')}
          disabled={!player.club_name}
        >
          {clubLabel}
        </button>
        <button
          className={`seg-btn ${scope === 'global' ? 'seg-btn--active' : ''}`}
        ...
        <div className="my-spot">
          <div>
            ...
            <div className="my-spot-rank tnum">
              #{myRank}
              {scopeSize > 0 && <span className="my-spot-of"> of {scopeSize}</span>}
            </div>
          </div>
      ...
      {!myRow && period === 'week' && !loading && (
        <div className="empty-spot">
          <div>You haven't logged any shots this week yet.</div>
          <div style={{ fontSize: 12, color: 'var(--text-mute)', marginTop: 4 }}>Head to Home and rack 'em up.</div>
        </div>
      )}
      ...
      {!loading && top.length > 0 && (
        <>
          <div className="label-sm" style={{ margin: '14px 4px 8px' }}>
            {scope === 'team' ? 'Top of the team' : scope === 'club' ? 'Top of the club' : 'Top of the board'}
          </div>
          <div className="board">
            {top.map((row, i) => (
              <Row key={row.id} row={row} rank={i + 1} isMe={row.id === player.id} period={period} />
            ))}
          </div>
        </>
      )}
        ...
        <>
          ...
          <div className="board">
            {aroundMe.map((row) => {
              ...
              return (
                <Row key={row.id} row={row} rank={idx + 1} isMe={row.id === player.id} period={period} />
              )
            })}
          </div>
        </>
      ...
      {!loading && rows.length === 0 && (
        <div className="empty-state">
          ...
          <div className="empty-sub">
            {scope === 'team' ? 'Be the first on your team.' : scope === 'club' ? 'Be the first in your club.' : 'Be the first to put up a number.'}
          </div>
        </div>
      )}
